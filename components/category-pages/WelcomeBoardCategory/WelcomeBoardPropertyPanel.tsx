...
interface WelcomeBoardPropertyPanelProps {
  post: Post | null
  onDownload?: (postId: string) => void
}

export function WelcomeBoardPropertyPanel({ post, onDownload }: WelcomeBoardPropertyPanelProps) {
        ...
        <div className="space-y-4 pt-4 pb-8 border-b">
          {post.description && (
            <div className="space-y-2">
              ...
              <p className="text-sm whitespace-pre-line break-words">{post.description}</p>
            </div>
          )}
          {post.producedAt && (
            <div className="space-y-2">
