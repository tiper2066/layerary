...
export async function GET(
  ...
  try {
    ...
    if (validatedQuery.format === 'svg') {
      ...
      // SVG 그대로 반환
      const encodedSvgFilename = encodeURIComponent(post.title)
      return new NextResponse(svgContent, {
        headers: {
          'Content-Type': 'image/svg+xml',
          'Content-Disposition': `attachment; filename*=UTF-8''${encodedSvgFilename}.svg`,
        },
      })
    } else {
      // SVG를 PNG 또는 JPG로 변환
      ...
      const encodedFilename = encodeURIComponent(post.title)
      return new NextResponse(new Uint8Array(imageBuffer), {
        headers: {
          'Content-Type': mimeType,
          'Content-Disposition': `attachment; filename*=UTF-8''${encodedFilename}.${validatedQuery.format}`,
        },
      })
    }
  } catch (error: any) {
